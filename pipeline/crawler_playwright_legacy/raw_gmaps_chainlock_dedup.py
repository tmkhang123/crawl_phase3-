...
DEFAULT_REGISTRY = PIPELINE_DIR / "config" / "brand_registry_phase1.csv"
DEFAULT_REGIONS = PIPELINE_DIR / "config" / "regions" / "regions_hybrid.json"
DEFAULT_OUTPUT = PIPELINE_DIR / "output" / "data_chainlock"
DEFAULT_ENV_FILES = [
    PROJECT_ROOT / ".env",
    PIPELINE_DIR / ".env",
]
...
def load_regions(path: Path, limit: int = 0) -> list[str]:
    ...
    for item in regions:
        if isinstance(item, str):
            ...
            lat = item.get("lat")
            lng = item.get("lng")
            zoom = item.get("zoom", 15)
            cell_km = item.get("cell_km")
            if not label or lat is None or lng is None:
                raise SystemExit(f"Bad region item in {path}: {item!r}")
            cell_part = f"|km{float(cell_km):g}" if cell_km is not None else ""
            normalized.append(f"grid:{label}|{float(lat):.6f},{float(lng):.6f}{cell_part}|z{float(zoom):g}")
        else:
            raise SystemExit(f"Bad region item in {path}: {item!r}")
    if limit:
...
def grid_region_parts(region: str) -> tuple[float, float, float, float | None] | None:
    match = re.match(
        r"^grid:(?P<label>[^|]+)\|(?P<lat>-?\d+(?:\.\d+)?),(?P<lng>-?\d+(?:\.\d+)?)"
        r"(?:\|km(?P<cell_km>\d+(?:\.\d+)?))?\|z(?P<zoom>\d+(?:\.\d+)?)$",
        region,
    )
    if not match:
        return None
    cell_km = float(match.group("cell_km")) if match.group("cell_km") else None
    return float(match.group("lat")), float(match.group("lng")), float(match.group("zoom")), cell_km


def build_search_url(config: ChainConfig, region: str) -> str:
    grid_parts = grid_region_parts(region)
    query = config.search_query
    encoded_query = urllib.parse.quote(query)
    if grid_parts:
        lat, lng, zoom, _ = grid_parts
        return f"https://www.google.com/maps/search/{encoded_query}/@{lat:g},{lng:g},{zoom:g}z"
    return "https://www.google.com/maps/search/" + urllib.parse.quote(f"{query} {region}")
...
def candidate_in_grid(region: str, lat: str | None, lng: str | None, args) -> bool:
    grid_parts = grid_region_parts(region)
    if not grid_parts or not clean_coord(lat) or not clean_coord(lng):
        return True
    center_lat, center_lng, _, cell_km = grid_parts
    if args.grid_max_distance_km > 0:
        max_distance_km = float(args.grid_max_distance_km)
    elif cell_km:
        max_distance_km = max(1.0, cell_km * float(args.grid_radius_multiplier))
    else:
        max_distance_km = 6.0
    return haversine_km(center_lat, center_lng, float(lat), float(lng)) <= max_distance_km
...
def seed_seen_from_csv(state: SeenState, path: Path) -> int:
    seeded = 0
    for row in read_csv(path):
        added = False
        ck = coord_key(row.get("lat"), row.get("lng"))
        if ck and ck not in state.coord_keys:
            state.coord_keys.add(ck)
            added = True
        nak = name_address_key(row.get("name"), row.get("address"))
        if nak and nak not in state.name_address_keys:
            state.name_address_keys.add(nak)
            added = True
        if added:
            seeded += 1
...
async def scrape_single_result(page, config: ChainConfig, region: str, seen: SeenState, stats: RegionStats, args) -> list[dict[str, str]]:
    ...
    if not candidate_in_grid(region, lat, lng, args):
        return stores
...
async def scrape_region(search_page, detail_page, config: ChainConfig, region: str, seen: SeenState, args) -> tuple[list[dict[str, str]], RegionStats]:
    stats = RegionStats()
    stores: list[dict[str, str]] = []
    search_url = build_search_url(config, region)
    ...
    if is_single:
        stores.extend(await scrape_single_result(search_page, config, region, seen, stats, args))
        return stores, stats
    ...
    for loc in locations:
        try:
            ...
            if not await seen.reserve_candidate(place_url, lat, lng):
                stats.duplicates_skipped += 1
                continue
            reserved_lat, reserved_lng = lat, lng
            ...
            if not candidate_in_grid(region, lat, lng, args):
                await seen.release_candidate(place_url, reserved_lat, reserved_lng)
                continue
    ...
    async with async_playwright() as p:
        ...
        while True:
            ...
            try:
                ...
                retry_limit = args.proxy_network_retry_limit if transient_network_error else args.block_retry_limit
                if args.proxy_enabled and retry_count < retry_limit:
                    await writer.mark_region_attempt(region, stats)
                    total.add(stats)
                    if args.defer_region_retries and deferred_retries is not None:
                        await deferred_retries.add(region, retry_count + 1)
                    ...
                    if is_transient_network_error(exc):
                        cooldown_seconds = float(args.proxy_network_error_cooldown_seconds)
                    else:
                        cooldown_seconds = max(
                            float(args.proxy_block_cooldown_seconds),
                            max(0.0, float(args.proxy_min_session_seconds) - session_age(session)),
                    ...
                    print(
                        f"[{worker_id}] {config.target_chain} | {region}: {retry_mode} "
                        f"attempt={retry_count + 1}/{retry_limit} {short_error(exc)}"
                    )
                elif transient_network_error and args.proxy_enabled:
                    await writer.mark_region_attempt(region, stats)
                    total.add(stats)
                    rotate_session = True
                    cooldown_seconds = float(args.proxy_network_error_cooldown_seconds)
                    print(
                        f"[{worker_id}] {config.target_chain} | {region}: failed_unresolved "
                        f"after={retry_count + 1} attempts; left pending for next run {short_error(exc)}"
                    )
                else:
                    stats.regions_done = 1
                    await writer.mark_region_done(task_key, region, stats)
...
async def run_chain(config: ChainConfig, regions: list[str], args) -> None:
    ...
    if args.seed_seen_dir:
        seed_file = args.seed_seen_dir / f"{clean_filename(config.target_chain)}.csv"
        seeded = seed_seen_from_csv(seen, seed_file) if seed_file.exists() else 0
        print(f"seed_seen={seeded} source={seed_file}", flush=True)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--seed-seen-dir", type=Path, default=None, help="Optional by-brand CSV directory used only to preload dedupe keys; seeded rows are not copied to output.")
    ...
    parser.add_argument("--grid-max-distance-km", type=float, default=0.0, help="Hard candidate radius for grid regions. 0 derives radius from cell size.")
    parser.add_argument("--grid-radius-multiplier", type=float, default=1.0, help="Derived grid radius = cell_km * multiplier.")
    ...
    parser.add_argument("--proxy-network-error-cooldown-seconds", type=float, default=5.0, help="Short cooldown for transient connection reset/closed errors.")
    parser.add_argument("--block-retry-limit", type=int, default=1)
    parser.add_argument("--proxy-network-retry-limit", type=int, default=3, help="Retries for transient proxy connection reset/closed errors. Unresolved regions remain pending.")
...
async def main_async(args: argparse.Namespace) -> None:
    if args.ckey_proxy_key and not args.proxy_config:
        print("ckey_proxy_preflight=checking", flush=True)
        try:
            proxy_config = await resolve_session_proxy_config(args)
        except Exception as exc:
            raise SystemExit(
                "CKEY proxy preflight failed before crawl: "
                f"{short_error(exc)}. Check that GMAPS_CKEY_PROXY_KEY is the active rotating-proxy key "
                "from CKEY proxy management, not an order code/account API key."
            ) from exc
        if not proxy_config:
            raise SystemExit("CKEY proxy preflight failed: CKEY returned no usable proxy configuration.")
        print(f"ckey_proxy_preflight=ok type={args.ckey_proxy_type}", flush=True)
...
def main() -> None:
    ...
    args.ckey_nhamang = (args.ckey_nhamang or os.environ.get("GMAPS_CKEY_NHAMANG") or "random").strip().lower()
    ...
    args.proxy_network_retry_limit = max(0, int(args.proxy_network_retry_limit))
    ...
    args.proxy_network_error_cooldown_seconds = max(0.0, float(args.proxy_network_error_cooldown_seconds))
    args.grid_max_distance_km = max(0.0, float(args.grid_max_distance_km))
    args.grid_radius_multiplier = max(0.1, float(args.grid_radius_multiplier))
